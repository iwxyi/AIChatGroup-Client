...
import { projectDeliberationSidebarModel, projectDeliberationSidebarRows } from './deliberationProjection';
...
describe('deliberationProjection', () => {
  ...
  it('groups deliberation artifacts for the sidebar model', () => {
    const chat = normalizeConversation({
      id: 'deliberation-sidebar-model',
      type: 'group',
      mode: 'group_discussion',
      sessionKind: { topology: 'group', family: 'analysis', scenarioId: 'opinion-review', surfaceProfile: 'text' },
      modeConfig: { freeSpeaking: true, allowInterruptions: true, allowPrivateThreads: false, allowDirectorInterventions: true, showRoleActions: true },
      ...
      scenarioState: {
        phase: 'deliberation',
        discussionMode: 'open',
        progress: [{ key: 'speeches', label: '审议进展', value: 4, target: 0 }],
        currentTurnActorId: 'b',
        deliberationEvidence: [{ id: 'evidence-saved', actorId: 'b', text: '三个合租案例中，公共区域冲突都来自临时占用。' }],
        deliberationMomentum: { support: 1, oppose: 0, inquiry: 1, review: 1, label: '追问边界' },
      },
      worldState: { phase: 'debating', mood: '', focus: '', recentEvent: '', conflictAxes: [] },
      directorControls: { allowSpeakAs: true, allowDirectorMode: true, allowEventInjection: true, allowForcedReply: true },
      createdAt: 1,
    ...
    const messages: Message[] = [{
      ...
      metadata: {
        deliberationArtifacts: {
          claims: [{ text: '公共区域需要可仲裁规则', stance: 'review', reason: '提出可执行边界', confidence: 0.82 }],
          issues: [{ text: '临时占用客厅时谁来仲裁？', targetActorId: 'b', reason: '仍缺少裁决路径', confidence: 0.8 }],
          verdicts: [{ text: '只靠人情维系不稳定', tendency: 'mixed', reason: '规则和善意需要分层', confidence: 0.78 }],
          summary: { text: '审议转向公共空间仲裁机制。' },
        },
      },
    }];

    const model = projectDeliberationSidebarModel(chat, [member('a', '甲'), member('b', '乙')], messages);

    expect(model?.phaseLabel).toBe('观点审议');
    ...
    expect(model?.summaryText).toContain('审议转向公共空间仲裁机制');
    expect(model?.momentum).toContain('追问边界');
  });
