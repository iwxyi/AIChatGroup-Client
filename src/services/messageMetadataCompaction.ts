...
function compactEvidenceHistory(value: unknown, maxItems: number) {
  return compactRecordArray(value, maxItems, (item) => omitEmpty({
    ...
    topicText: compactText(item.topicText, MAX_TEXT.reason),
    promiseId: item.promiseId,
    promiseText: compactText(item.promiseText, MAX_TEXT.reason),
    promiseKind: item.promiseKind,
    supersedesText: compactText(item.supersedesText, MAX_TEXT.reason),
    anchorId: item.anchorId,
    mergedAnchorIds: compactStringArray(item.mergedAnchorIds, 4, 80),
    secretId: item.secretId,
    phraseId: item.phraseId,
    ritualId: item.ritualId,
    participantIds: compactStringArray(item.participantIds, 6, 80),
    visibility: item.visibility,
    firstSaidBy: item.firstSaidBy,
    currentAddress: compactText(item.currentAddress, 80),
    privateAddress: compactText(item.privateAddress, 80),
    publicAddress: compactText(item.publicAddress, 80),
    forbiddenAddresses: compactStringArray(item.forbiddenAddresses, 4, 80),
    initiatedBy: item.initiatedBy,
    urgency: item.urgency,
    items: compactRecordArray(item.items, 4, (profileItem) => omitEmpty({
      kind: profileItem.kind,
      text: compactText(profileItem.text, MAX_TEXT.reason),
      evidence: compactText(profileItem.evidence, MAX_TEXT.evidence),
      sourceMessageIds: compactStringArray(profileItem.sourceMessageIds, 3, 80),
    ...
    phase: item.phase,
    style: item.style,
    inferredStyle: item.inferredStyle,
    ...
    content: compactText(item.content, MAX_TEXT.reason),
    reason: compactText(item.reason, MAX_TEXT.reason),
    evolution: compactStringArray(item.evolution, 3, MAX_TEXT.reason),
    evidence: compactStringArray(item.evidence, 2, MAX_TEXT.evidence),
    lifecycleEvidence: compactStringArray(item.lifecycleEvidence, 2, MAX_TEXT.evidence),
    adaptations: compactStringArray(item.adaptations, 3, MAX_TEXT.reason),
    sourceEventIds: compactStringArray(item.sourceEventIds, 3, 80),
...
function compactCompanionshipContext(context: NonNullable<NonNullable<MessageMetadata['runtimeDecision']>['companionshipContext']>) {
  return omitEmpty({
    ...
    sharedPhraseHistory: compactEvidenceHistory(context.sharedPhraseHistory, 3),
