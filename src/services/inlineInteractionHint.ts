...
4. Do not use intentionalRepeat=true for accidental template drift. If you are merely falling back into the same opener, explanation scaffold, punctuation habit, or generic answer shape, set false and rewrite with a different discourse move.`;
  ...
  const mediaRules = (shouldIncludeMediaDecision
    ? `\n\nRules for mediaDecision:\n1. mediaDecision is included because this turn has an explicit media request. If the request is not actually for media, set shouldGenerate=false.\n${mediaCapabilities.image ? `2. For image requests, set image.shouldGenerate=true only when the visible reply is sending/showing/generating an image. Write a professional text prompt centered on the requested subject, grounded in speaker identity and current context; avoid generic stock-photo wording, impossible anatomy, watermarks, URLs, and text overlays.\n3. image.prompt must be production-ready, not a short caption. Adaptively choose, add, remove, and combine only the layers relevant to this task. These are optional layers, not a fixed checklist: subject identity, scene/action, composition/layout, camera or rendering style, lighting, color grade, texture/material details, mood, quality level, and negative constraints. For text-first graphics, UI-like images, OCR/reading tasks, repair, sharpening, or faithful edits, prioritize accuracy/readability/preservation and do not add cinematic lighting, emotional mood, lens, or luxury style terms unless the user asked for them.\n4. Preserve user-specified style terms and concrete objects. If the user only gives a terse theme, expand it into a specific visual plan without inventing a different subject or goal.\n5. If the user asks for image shape or quality in natural language, output aspectRatio as one of 1:1, 2:3, 3:2, 3:4, 4:3, 4:5, 5:4, 9:16, 16:9, 21:9, and imageSize as one of 1K, 2K, 4K. Leave them null when the user did not ask.\n6. Do not pretend an image exists unless image.shouldGenerate=true. altText should be concise and specific.\n7. imageReferenceRegistry below is a lightweight list of recent chat images. Do not use every image by default. Select targetImageIds/referenceImageIds/styleImageIds only when the user instruction, latest turn, caption/alt text, or message order clearly identifies them. If several images could be meant and the difference matters, ask a short clarifying question in content and set image.shouldGenerate=false.\n8. targetImageIds means images to edit or vary; referenceImageIds means content/object references; styleImageIds means style references. IDs must come from imageReferenceRegistry.refId. Never output URLs, base64, markdown image links, or binary data.\nImage reference registry:\n${JSON.stringify(imageReferenceRegistry)}\n` : ''}${mediaCapabilities.audio ? '9. For audio requests, set audio.shouldGenerate=true only when this exact text should be spoken. audio.text must not add new facts.\n' : ''}10. Never output URLs, base64, markdown image links, or binary data.`
